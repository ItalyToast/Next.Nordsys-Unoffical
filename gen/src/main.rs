...
impl Field {
    ...
    pub fn with_default(name : &str, datatype : &str, default : &str) -> Field {
        Field {
...
fn gen_from_next_js() {
    ...
    result.push_str("\r\n");
    result.push_str("fn default_bool() -> String { \"false\".to_string() }");
    result.push_str("fn default_int() -> String { \"0\".to_string() }");
    ...
    for class_cap in class_regex.captures_iter(&data) {
        ...
        for field_cap in fields_regex.captures_iter(&class)  {
            let mut f = parse_field(field_cap.get(0).unwrap().as_str());
